...
// const destPath = '/home/dalsh/Development/atlassian-labs/atlaspack/packages/core/atlaspack/src/transformers'

// for (const packageName of fs.readdirSync(path.join(__root, 'packages', 'transformers'))) {
//   console.log(packageName)
//   const packagePathCoreDest = path.join(destPath, packageName)

//   const packagePath = path.join(__root, 'packages', 'transformers', packageName)
//   const packagePathDest = path.join(__root, 'packages', 'shims', `transformer-${packageName}`)
//   const packageJsonPath = path.join(packagePath, 'package.json')
...
//   const [namespce, pkg] = packageName.split(/-(.*)/s).filter(v => v)
