...
async function countTSSuppressions(basePath: string): Promise<number> {
    const count = Number((await $`rg --no-heading -o '@ts-expect-error' ${basePath} | wc -l`.text()).trim());
    return count;
}

async function saveMetricsToGitNote(commitHash: string, count: number): Promise<void> {
...
async function showTSSuppressions(days: number = 30) {
    try {
        ...
        const commits = (await $`git log --since="${days} days ago" --format="%H %at" --reverse`.text())
            .trim()
            .split('\n')
        ...
        for (const commit of commits) {
            const commitDate = new Date(commit.timestamp * 1000).toISOString().split('T')[0];
            
            try {
                // Try to get the git note for this commit
                const note = (await $`git notes show ${commit.hash} 2>/dev/null`.text()).trim();
                
                // Parse the METRICS line if it exists
                const metricsLine = note.split('\n').find(line => line.startsWith('METRICS: '));
                if (metricsLine) {
                    try {
...
async function backfillTSSuppressions(basePath: string, days: number = 30) {
    try {
        ...
        try {
            ...
            const commits = (await $`git log --since="${days} days ago" --format="%H %at" --reverse --first-parent`.text())
                .trim()
                .split('\n')
                .filter(line => line.length > 0)
                .map(line => {
            ...
            for (const commit of commits) {
                ...
                try {
                    ...
                    try {
                        existingNote = (await $`git notes show ${commit.hash}`.text()).trim();
                        hasMetrics = existingNote.split('\n').some(line => line.startsWith('METRICS: '));
                    } catch (noteError) {
                    ...
                    try {
                        ...
                        console.error(`  Failed to checkout commit ${commit.hash}:`, checkoutError);
                        continue;
                    }
                    
                    // Count TS suppressions for this commit using the worktree path
                    try {
                        const count = Number((await $`cd ${worktreePath} && rg --no-heading -o '@ts-expect-error' ${basePath} | wc -l`.text()).trim());
                        ...
                        console.error(`  Failed to count suppressions for commit ${commit.hash}:`, countError);
                    }
                } catch (error) {
            ...
            try {
                ...
                console.error(`Warning: Could not remove worktree ${worktreePath}:`, cleanupError);
            }
        }
