...
type TransformOptions = Parameters<
  ConstructorParameters<typeof Transformer>[0]['transform']
>[0];
...
export default new Transformer({
  ...
  async transform({asset, options, config, resolve}) {
    ...
    if (result.map != null) {
      ...
      map.addVLQMap({
        ...rawMap,
        sources: rawMap.sources.map((s: any) =>
...
function urlPlugin({asset}: Pick<TransformOptions, 'asset'>) {
  return {
...
function resolvePathPlugin({
  asset,
  resolve,
}: Pick<TransformOptions, 'asset'> & Pick<TransformOptions, 'resolve'>) {
  return {
    install(less: any, pluginManager: any) {
