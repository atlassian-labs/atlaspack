...
impl VisitMut for GlobalReplacer<'_> {
  fn visit_mut_expr(&mut self, node: &mut Expr) {
    ...
    match id.sym.to_string().as_str() {
      ...
      "Buffer" => {
        let specifier = swc_core::atoms::Atom::from("buffer");
        if self.update_binding(id, |_| {
          Member(MemberExpr {
      ...
      "__filename" => {
        self.update_binding(id, |this| {
          ...
          Lit(ast::Lit::Str(swc_core::atoms::Atom::from(filename).into()))
        });
      }
      "__dirname" => {
        self.update_binding(id, |this| {
          ...
          Lit(ast::Lit::Str(swc_core::atoms::Atom::from(dirname).into()))
        });
      }
      "global" => {
        if !self.scope_hoist {
