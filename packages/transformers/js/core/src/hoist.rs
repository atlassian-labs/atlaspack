...
pub fn hoist(
  ...
  collect: &Collect,
) -> Result<(HoistResult, Vec<Diagnostic>), Vec<Diagnostic>> {
  let mut hoist = Hoist::new(module_id, unresolved_mark, collect);

  module.visit_mut_with(&mut hoist);
  ...
  Ok((hoist.get_result(), diagnostics))
}
...
impl VisitMut for Hoist<'_> {
  fn visit_mut_module(&mut self, node: &mut Module) {
    let module_items: &mut Vec<ModuleItem> = node.body.as_mut();
        ...
        ModuleItem::ModuleDecl(decl) => {
          match decl {
                ...
                for specifier in &export.specifiers {
                  match specifier {
                    ...
                    ExportSpecifier::Default(default) => {
                      self.re_exports.push(ImportedSymbol {
                        source: src.value.clone(),
                        local: default.exported.sym.clone(),
            ...
            ModuleDecl::ExportDefaultDecl(export) => {
              if let Some(class_expr) = export.decl.as_mut_class() {
                class_expr.class.visit_mut_with(self);

                let decl = Decl::Class(ClassDecl {
                  ident: if self.collect.should_wrap && class_expr.ident.is_some() {
                    class_expr.ident.clone().unwrap()
                  } else {
            ...
            ModuleDecl::ExportDecl(export) => {
              ...
              self
                .module_items
                .push(ModuleItem::Stmt(Stmt::Decl(export.decl.clone())));
            }
            item => {
              item.visit_mut_with(self);

              self.module_items.push(ModuleItem::ModuleDecl(item.clone()))
            }
          }
        }
        ModuleItem::Stmt(stmt) => {
          match stmt {
            Stmt::Decl(decl) => {
              match decl {
                ...
                item => {
                  ...
                  self
                    .module_items
                    .push(ModuleItem::Stmt(Stmt::Decl(item.clone())));
                }
              }
            }
            ...
            item => {
              ...
              self.module_items.push(ModuleItem::Stmt(item.clone()));
            }
          }
        }
      }
  ...
  visit_mut_fn!(visit_mut_function, Function);
  visit_mut_fn!(visit_mut_class, Class);
  visit_mut_fn!(visit_mut_getter_prop, GetterProp);
  visit_mut_fn!(visit_mut_setter_prop, SetterProp);

  fn visit_mut_expr(&mut self, node: &mut Expr) {
    match node {
      Expr::OptChain(opt) => {
        match opt.base.as_mut() {
      ...
      Expr::Member(member) => {
        if !self.collect.should_wrap {
          if match_member_expr(member, vec!["module", "exports"], self.unresolved_mark) {
            ...
            *node = Expr::Ident(self.get_export_ident(member.span, &"*".into()));
            return;
          }

          if match_member_expr(member, vec!["module", "hot"], self.unresolved_mark) {
            *node = Expr::Lit(Lit::Null(Null { span: member.span }));
            return;
          }
        }
        ...
        match &*member.obj {
          Expr::Ident(ident) => {
            ...
            }) = self.collect.imports.get(&id!(ident))
            {
                ...
                && !self.collect.non_static_requires.contains(source)
              {
                if *kind == ImportKind::DynamicImport {
                  ...
                  *node = Expr::Ident(self.get_import_ident(
                    member.span,
                    source,
                    &key,
              ...
              && !self.collect.should_wrap
            {
              ...
              *node = Expr::Ident(self.get_export_ident(member.span, &key));
              return;
            }
          }
          Expr::Call(_) => {
      ...
      Expr::Call(ref call) => {
        ...
        if let Some(source) = match_import(node) {
          ...
          *node = Expr::Ident(Ident::new(name, call.span, call.ctxt));
          return;
        }

        if let Some((source_true, source_false)) = match_import_cond(node, self.collect.ignore_mark)
        {
          let name: JsWord =
          ...
          *node = Expr::Ident(Ident::new_no_ctxt(name, call.span));
          return;
        }
      }
      Expr::This(this) => {
        if !self.in_function_scope {
          ...
          if self.collect.is_esm {
            *node = Expr::Ident(get_undefined_ident(self.unresolved_mark));
            ...
            self.self_references.insert("*".into());
            *node = Expr::Ident(self.get_export_ident(this.span, &"*".into()));
            return;
          }
        }
      }
        ...
        }) = self.collect.imports.get(&id!(ident))
        {
          if kind == &ImportKind::Import && specifier != "*" {
            ...
            *node = Expr::Seq(SeqExpr {
              span: ident.span,
              exprs: vec![0.into(), Box::new(Expr::Ident(ident.clone()))],
            });
  ...
  fn visit_mut_ident(&mut self, node: &mut Ident) {
      ...
      if !self.collect.non_static_requires.contains(source) {
        if *kind == ImportKind::DynamicImport {
          ...
          if self.collect.non_const_bindings.contains_key(&id!(node)) {
            *node = self.get_require_ident(&node.sym);
            return;
          }

          *node = self.get_import_ident(node.span, source, specifier, loc.clone(), *kind);
          return;
        }
      }
    }
    ...
    if let Some(exported) = self.collect.exports_locals.get(&id!(node)) {
      ...
      if self.collect.should_wrap {
        ...
        *node = self.get_export_ident(node.span, exported);
        return;
      }
    }
    ...
    {
      self.self_references.insert("*".into());
      *node = self.get_export_ident(node.span, &"*".into());
      return;
    }

    if node.sym == js_word!("global") && is_unresolved(node, self.unresolved_mark) {
      *node = Ident::new("$parcel$global".into(), node.span, node.ctxt);
      return;
    }
      ...
      && !self.collect.should_wrap
    {
      ...
      *node = Ident::new(new_name, node.span, node.ctxt);
    }
  }
  ...
  fn visit_mut_assign_expr(&mut self, node: &mut AssignExpr) {
    ...
    if let Some(SimpleAssignTarget::Member(member)) = node.left.as_mut_simple() {
      ...
      if match_member_expr(member, vec!["module", "exports"], self.unresolved_mark) {
        ...
        node.left = AssignTarget::Simple(SimpleAssignTarget::Ident(ident));
      ...
      if is_cjs_exports {
        ...
        node.left = if self.collect.static_cjs_exports {
  ...
  fn visit_mut_prop(&mut self, node: &mut Prop) {
    if let Prop::Shorthand(ident) = node {
      ...
      *node = Prop::KeyValue(KeyValueProp {
        key: PropName::Ident(IdentName::new(key, DUMMY_SP)),
  ...
  fn visit_mut_prop_name(&mut self, node: &mut PropName) {
    if let PropName::Computed(_key) = node {
      node.visit_mut_children_with(self);
    }
  }
  ...
  fn visit_mut_object_pat_prop(&mut self, node: &mut ObjectPatProp) {
    ...
    if let ObjectPatProp::Assign(assign) = node {
      let key = assign.key.sym.clone();

      assign.key.visit_mut_with(self);
      assign.value.visit_mut_with(self);

      *node = ObjectPatProp::KeyValue(KeyValuePatProp {
        key: PropName::Ident(IdentName::new(key, DUMMY_SP)),
        value: Box::new(match assign.value.as_mut() {
          Some(value) => Pat::Assign(AssignPat {
            left: Box::new(Pat::Ident(assign.key.clone())),
            right: value.clone(),
            span: DUMMY_SP,
          }),
          None => Pat::Ident(assign.key.clone()),
        }),
      });
...
mod tests {
  ...
  #[test]
  fn visit_cjs_export_non_static() {
    let (_collect, code, _hoist) = parse(
      r#"
        exports[foo] = 2;
        exports.bar = 3;
