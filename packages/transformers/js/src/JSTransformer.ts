...
export default new Transformer({
  async loadConfig({config, options}) {
    ...
    if (config.isSource) {
      ...
      let tsconfig = await config.getConfigFrom<TSConfig>(options.projectRoot + '/index', ['tsconfig.json', 'jsconfig.json']);
      ...
      let convertDiagnostic = (diagnostic: any) => {
        ...
        let res: Diagnostic = {
          ...
          codeFrames: [
            {
              filePath: asset.filePath,
              codeHighlights: diagnostic.code_highlights?.map((highlight: any) =>
                convertSourceLocationToHighlight(
