...
export async function load(config: Config, options: PluginOptions, logger: PluginLogger): Promise<BabelConfigResult | null | undefined> {
  ...
  let partialConfig: {
    [key: string]: any
  } | null | undefined = await babelCore.loadPartialConfigAsync(babelOptions);
...
async function buildDefaultBabelConfig(options: PluginOptions, config: Config): Promise<BabelConfigResult | null | undefined> {
...
function definePluginDependencies(config: Config, babelConfig: BabelConfig | null | undefined, options: PluginOptions) {
  if (babelConfig == null) {
    return;
  }
...
async function warnOnRedundantPlugins(fs: FileSystem, babelConfig: {
  [key: string]: any
}, logger: PluginLogger) {
  if (babelConfig == null) {
    return;
...
async function getCodeHighlights(fs: FileSystem, filePath: any, redundantPresets: Set<any | string>) {
