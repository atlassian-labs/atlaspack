 ...
 */
export default async function getFlowOptions(config: Config, options: PluginOptions): Promise<BabelConfig | null | undefined> {
  if (!config.isSource) {
    return null;
  }

  // Only add flow plugin if `flow-bin` is listed as a dependency in the root package.json
  let conf = await config.getConfigFrom<PackageJSON>(options.projectRoot + '/index', ['package.json']);
