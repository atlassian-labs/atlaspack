...
function createClientPipeTransport(
  ...
  return new Promise((resolve: (
    ...
    server.listen(pipeName, () => {
      ...
      resolve({
        close() {
          return new Promise((res: (result: Promise<undefined> | undefined) => void, rej: (error?: any) => void) => {
            server.close((e) => {
              if (e) rej(e);
              else res();
            });
...
export function createServer(filename: string, setup: (connection: MessageConnection) => void): Promise<{
  close: () => Promise<void>
}> {
