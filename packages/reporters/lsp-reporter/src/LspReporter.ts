...
let workspaceDiagnostics: DefaultMap<string, Array<Diagnostic>> = new DefaultMap(() => []);

const getWorkspaceDiagnostics = (): Array<PublishDiagnostic> => [...workspaceDiagnostics].map(([uri, diagnostics]: [any, any]) => ({uri, diagnostics}));

let server;
let connections: Array<MessageConnection> = [];

let bundleGraphDeferrable =
  makeDeferredWithPromise<BundleGraph<PackagedBundle> | null | undefined>();
...
function getDiagnosticsUnusedExports(bundleGraph: BundleGraph<PackagedBundle>, document: string): Array<Diagnostic> {
...
function getImporters(bundleGraph: BundleGraph<PackagedBundle>, document: string): Array<DocumentUri> | null {
  let filename = url.fileURLToPath(document);
