...
export default new Reporter({
  async report({event, options}) {
    ...
    let bundlesByTarget: DefaultMap<string /* target name */, Array<PackagedBundle>> = new DefaultMap(() => []);
    ...
    await Promise.all(
      [...bundlesByTarget.entries()].map(async ([targetName, bundles]: [any, any]) => {
...
async function getBundleData(bundles: Array<PackagedBundle>, options: PluginOptions): Promise<BundleData> {
