...
import type {Asset, BundleGraph, Dependency, NamedBundle, PackagedBundle, PluginOptions} from '@atlaspack/types';
...
export type HMRMessage = {
  ...
  diagnostics: {
    ...
    html: Array<Partial<Flow.Diff<AnsiDiagnosticResult, {
      codeframe: string
    }>>>
  }
};
...
export default class HMRServer {
  ...
  bundleGraph: BundleGraph<PackagedBundle> | BundleGraph<NamedBundle> | null = null;
  ...
  async start() {
    ...
    if (!server) {
      ...
      this.options.addMiddleware?.((req: Request, res: Response) => this.handle(req, res));
    }
