...
import {runJsOptimizer} from '@atlaspack/rust';
...
module.exports = new Optimizer<empty, BundleConfig>({
  ...
  optimize({
    ...
    try {
      ...
      logger.warn({
        origin: 'atlaspack-optimizer-js',
        message: `JS optimizer failed for ${bundle.name}: ${err.message}`,
        stack: err.stack,
      });
    }
    return {contents, map: originalMap};
