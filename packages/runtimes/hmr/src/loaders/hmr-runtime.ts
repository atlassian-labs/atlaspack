  ...
  ws.onmessage = async function (event: {
    ...
    if (data.type === 'reload') {
      ...
      if (handled) {
        ...
        let processedAssets: Record<string, any> = ({} /*: {|[string]: boolean|} */);
...
function createErrorOverlay(diagnostics: Array<Partial<Flow.Diff<AnsiDiagnosticResult, {
  codeframe: string
}>>>) {
...
function getParents(bundle: ParcelRequire, id: string) /*: Array<[ParcelRequire, string]> */ {
...
function hmrDownload(asset: HMRAsset) {
  if (asset.type === 'js') {
    if (typeof document !== 'undefined') {
      ...
      return new Promise((resolve: (result: Promise<HTMLScriptElement> | HTMLScriptElement) => void, reject: (error?: any) => void) => {
...
function hmrApply(bundle: ParcelRequire /*: ParcelRequire */, asset: HMRAsset /*:  HMRAsset */) {
  var modules = bundle.modules;
  if (!modules) {
    return;
  }
...
function hmrDispose(bundle: ParcelRequire /*: ParcelRequire */, id: string /*: string */) {
  var cached = bundle.cache[id];
  bundle.hotData[id] = {};
...
function hmrAccept(bundle: ParcelRequire /*: ParcelRequire */, id: string /*: string */) {
  // Execute the module.
  bundle(id);
