...
digraph simplified_graph {
  ...
  "page1.js" -> "library2.js";
    ...
    const page1Key = contentKeysByPath.get('page1.js') ?? '';
    const library1Key = contentKeysByPath.get('library1.js') ?? '';
    const library2Key = contentKeysByPath.get('library2.js') ?? '';
    assert.deepEqual(
...
digraph simplified_graph {
  ...
  "index.html" -> "page1.js";
    ...
    const contentKeysByPath = getContentKeysByPath(simplifiedGraph);
    const bundleReferences = result.getBundleReferences();
    const indexKey = contentKeysByPath.get('index.html') ?? '';
    const page1Key = contentKeysByPath.get('page1.js') ?? '';
    assert.deepEqual(
...
digraph simplified_graph {
  ...
  "index.js" -> "child.png";
    ...
    const contentKeysByPath = getContentKeysByPath(simplifiedGraph);
    const bundleReferences = result.getBundleReferences();
    const indexKey = contentKeysByPath.get('index.js') ?? '';
    const childKey = contentKeysByPath.get('child.png') ?? '';
    assert.deepEqual(
      bundleReferences.get(indexKey)?.map((r) => r.assetContentKey) ?? [],
      [childKey],
    );
