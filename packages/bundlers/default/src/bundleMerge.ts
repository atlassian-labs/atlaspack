...
function getBundlesForBundleGroup(bundleGraph: IdealBundleGraph, bundleGroupId: NodeId): number {
  let count = 0;
...
let getBundleOverlap = (sourceBundlesA: Set<NodeId>, sourceBundlesB: Set<NodeId>): number => {
...
function checkBundleThreshold(bundleA: MergeCandidate, bundleB: MergeCandidate, threshold: number): boolean {
  return (
    getBundleOverlap(
...
let hasSuitableBundleGroup = memoize(
  (bundleGraph: IdealBundleGraph, bundle: Bundle, minBundlesInGroup: number): boolean => {
...
function getMergeClusters(graph: ContentGraph<NodeId, EdgeType>, candidates: Map<NodeId, EdgeType>): Array<Array<NodeId>> {
...
function getPossibleMergeCandidates(bundleGraph: IdealBundleGraph, bundles: Array<NodeId>): Array<[MergeCandidate, MergeCandidate]> {
