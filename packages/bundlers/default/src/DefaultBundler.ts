...
type BundlerConfig = Partial<Record<BuildMode, BaseBundlerConfig>> & BaseBundlerConfig;
...
function decorateLegacyGraph(idealGraph: IdealGraph, bundleGraph: MutableBundleGraph): void {
  ...
  for (let [bundleNodeId, idealBundle] of idealBundleGraph.nodes.entries()) {
    ...
    let bundleGroups: Array<BundleGroup> = [];
    let bundleGroup;
    let bundle;
...
function createIdealGraph(
  ...
  let assetReference: DefaultMap<Asset, Array<[Dependency, Bundle]>> = new DefaultMap(() => []);
  ...
  // asset index
  number, typeof bundleRootEdgeTypes[keyof typeof bundleRootEdgeTypes]> = new Graph();
  ...
  let manualBundleToInternalizedAsset: DefaultMap<NodeId, Array<Asset>> = new DefaultMap(() => []);
  ...
  assetGraph.traverse(
    {
      enter(node: BundleGraphTraversable | {
...
function resolveModeConfig(config: BundlerConfig, mode: BuildMode): BaseBundlerConfig {
...
async function loadBundlerConfig(config: Config, options: PluginOptions, logger: PluginLogger): Promise<ResolvedBundlerConfig> {
...
function getEntryByTarget(bundleGraph: MutableBundleGraph): DefaultMap<string, Map<Asset, Dependency>> {
  // Find entries from assetGraph per target
  let targets: DefaultMap<string, Map<Asset, Dependency>> = new DefaultMap(
    () => new Map(),
  );
  bundleGraph.traverse({
    enter(node: BundleGraphTraversable, context: {
      readonly type: 'asset',
      value: Asset
    } | null | undefined | {
      readonly type: 'dependency',
