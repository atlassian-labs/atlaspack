...
export const exec: (command: string, options?: child_process.execOpts) => Promise<{
...
export function npmSpecifierFromModuleRequest(moduleRequest: ModuleRequest): string {
  return moduleRequest.range != null
    ? [moduleRequest.name, moduleRequest.range].join('@')
    : moduleRequest.name;
}

export function moduleRequestsFromDependencyMap(
  dependencyMap: {
    [key: string]: string
  },
): Array<ModuleRequest> {
...
export async function getConflictingLocalDependencies(fs: FileSystem, name: string, local: FilePath, projectRoot: FilePath): Promise<{
  json: string,
  filePath: FilePath,
  fields: Array<string>
} | null | undefined> {
