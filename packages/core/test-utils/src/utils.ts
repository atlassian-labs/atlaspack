...
export function sleep(ms: number): Promise<void> {
  return new Promise((resolve: (result: Promise<undefined> | undefined) => void) => setTimeout(resolve, ms));
}
...
export function findAsset(bundleGraph: BundleGraph<PackagedBundle>, assetFileName: string): Asset | null | undefined {
...
export function mergeParcelOptions(optsOne: InitialAtlaspackOptions, optsTwo?: InitialAtlaspackOptions | null): InitialAtlaspackOptions {
  if (!optsTwo) {
    return optsOne;
  }
...
export async function bundle(entries: FilePath | Array<FilePath>, opts?: InitialAtlaspackOptions): Promise<BundleGraph<PackagedBundle>> {
...
export async function getNextBuildSuccess(b: Parcel): Promise<BuildSuccessEvent> {
...
export function getBundleData(bundleGraph: BundleGraph<PackagedBundle>, inputDir: string): {
  name: string,
  type: string,
  assets: string[]
}[] {
  const byAlphabet = (a: string, b: string) => (a.toLowerCase() < b.toLowerCase() ? -1 : 1);
...
function prepareBrowserContext(bundle: PackagedBundle, globals: unknown): {
  ctx: vm.Context,
...
function createWorkerClass(filePath: FilePath) {
  return class Worker extends EventEmitter {
    ...
    addEventListener(evt, callback: any) {
      super.on(evt, callback);
    }

    removeEventListener(evt, callback: any) {
      super.removeListener(evt, callback);
    }
  };
}

function prepareWorkerContext(filePath: FilePath, globals: unknown): {
  ctx: vm.Context,
  promises: Array<Promise<unknown>>
} {
  let promises: Array<Promise<unknown>> = [];
...
export async function runESM(
  ...
  function entry(specifier: string, referrer: {
    identifier: string
...
export function request(file: string, port: number, client: typeof http | typeof https = http): Promise<string> {
