...
export async function runBenchmark(options: BenchmarkOptions): Promise<BenchmarkResult> {
  const {samples: sampleCount, target, name, warmupRuns = 1, buildOptions} = options;
  ...
  for (let i = 0; i < sampleCount; i++) {
    ...
    // Track memory usage during build with high resolution
    ...
    try {
      ...
      try {
        ...
        await buildFixture(target, {
          ...
          instanceId: `sample_${i}`,
        });

        // Get detailed phase metrics if available - try different possible instance IDs
        let phaseMetrics = getPhaseMetrics(`sample_${i}`);

        // If that doesn't work, try to find any available metrics
        if (!phaseMetrics || phaseMetrics.phases.length === 0) {
          const globalMetrics = global.__phaseTrackingMetrics;
          if (globalMetrics) {
            const availableIds = Object.keys(globalMetrics);
            console.log(`[Runner] Available instance IDs:`, availableIds);
            if (availableIds.length > 0) {
              // Use the most recent metrics
              const latestId = availableIds[availableIds.length - 1];
              phaseMetrics = globalMetrics[latestId];
              console.log(`[Runner] Using metrics from instance: ${latestId}`);
            }
          }
        }
        ...
        if (phaseMetrics && phaseMetrics.phases.length > 0) {
          ...
          throw new Error('Phase tracking not available, fall back to basic tracking');
        }
      } catch (reporterError) {
        // Fallback to basic tracking if phase tracking fails
        console.warn('Phase tracking failed, using basic tracking:', reporterError.message);

        const buildStartMemory = getMemoryUsage();

        await buildFixture(target, buildOptions);

        const buildEnd = performance.now();
        const buildEndMemory = getMemoryUsage();
        ...
        // Create simple phase metrics
        const buildPhase: PhaseMetrics = {
          name: 'build',
          duration: buildEnd - buildStart,
          startTime: buildStart,
          endTime: buildEnd,
          startMemory: buildStartMemory,
          endMemory: buildEndMemory,
          memoryPeak: memoryPeak,
          memorySnapshots: memorySnapshots.length,
          memoryStats: calculateMemoryStatsFromSnapshots(memorySnapshots),
          gcCount: 0,
          gcTime: 0,
        };
        ...
        const sample: BenchmarkSample = {
          ...
          memoryPeak: memoryPeak,
          memoryEnd: endMemory,
          overallMemoryStats: calculateMemoryStatsFromSnapshots(memorySnapshots),
          globalMemoryPeak: memoryPeak,
        };
  ...
  const phaseStats: {[phaseName: string]: {duration: DetailedMemoryStats; memoryPeak: DetailedMemoryStats}} = {};
...
function calculateMemoryStatsFromSnapshots(snapshots: MemorySnapshot[]): MemoryStats | null {
