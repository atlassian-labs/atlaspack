...
// separate stylesheet that the @compiled/parcel-optimizer links into <head>.
//
// cssMapScoped is most valuable for editor-style components that need to
// style many nested children with a single wrapper class. This fixture
// models a realistic editor panel:
//   - `.editor .panel` and `.editor .panel-title` — nested descendant
//     selectors which atomic CSS struggles with.
//   - Two override variants (`panelDangerStyles`, `panelDangerStylesNew`)
//     share variant keys but must produce DIFFERENT class names.
//   - `@media` query inside a variant.
//   - `@keyframes` via `keyframes()` — global identifier, not scoped.
...
// Old danger styling — same variant key as the "new" version below.
// Also exercises **comma-separated selector lists** at the variant level:
// `.editor .panel, .editor .panel-title` must scope EACH selector
// independently to `.cc-<hash> .editor .panel, .cc-<hash> .editor .panel-title`.
...
// New danger styling — overlapping variant key but different class hash,
...
const page = (
  <>
    {/* Default panel — base styling only. Includes a tabindex so we can
        verify the :focus pseudo, and a .panel-footer for descendant nesting. */}
    ...
    {/* Danger panel — base + override (uses old danger style). */}
    <div
    ...
    {/* Danger new panel — base + old + new overrides stacked via array. */}
    <div
