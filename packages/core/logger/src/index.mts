...
export {instrument, instrumentAsync} from './tracer.mts';

class Logger {
  #logEmitter: ValueEmitter<LogEvent> = new ValueEmitter();

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  onLog(cb: (event: LogEvent) => any): IDisposable {
  ...
  error(input: Diagnostifiable, realOrigin?: string): void {
    ...
    if (typeof realOrigin === 'string') {
      diagnostic = Array.isArray(diagnostic)
        ...
        : {
            // @ts-expect-error Looks like anyToDiagnostic might return a Diagnostic | Array<Diagnostic>
            ...diagnostic,
            origin: realOrigin,
          };
    }
...
function messagesToDiagnostic(
  ...
  if (messages.length === 1 && messages[0] instanceof Error) {
    ...
    if (Array.isArray(diagnostic)) {
      ...
      return {
        // @ts-expect-error Maybe be an object but missing types
        ...diagnostic,
        skipFormatting: true,
      };
    }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function joinLogMessages(messages: Array<any>): string {
  return messages
