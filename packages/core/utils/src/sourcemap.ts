...
export function matchSourceMappingURL(contents: string): null | RegExp.matchResult {
  return contents.match(SOURCEMAP_RE);
}

export async function loadSourceMapUrl(fs: FileSystem, filename: string, contents: string): Promise<{
  filename: string,
  map: any,
  url: string
} | null | undefined> {
...
export function remapSourceLocation(loc: SourceLocation, originalMap: SourceMap): SourceLocation {
  let {
    filePath,
