...
export function measureStreamLength(stream: Readable): Promise<number> {
  return new Promise((resolve: (result: Promise<number> | number) => void, reject: (error?: any) => void) => {
    let length = 0;
...
export function bufferStream(stream: Readable): Promise<Buffer> {
  return new Promise((resolve: (result: Promise<Buffer> | Buffer) => void, reject: (error?: any) => void) => {
    let buf = Buffer.from([]);
    stream.on('data', (data) => {
...
export function fallbackStream(stream: Readable, fallback: () => Readable): Readable {
