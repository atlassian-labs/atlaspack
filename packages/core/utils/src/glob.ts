...
export function isGlobMatch(filePath: FilePath, glob: Glob | Array<Glob>, opts?: Options): any {
...
export function globMatch(values: Array<string>, glob: Glob | Array<Glob>, opts?: Options): Array<string> {
  glob = Array.isArray(glob)
    ? glob.map(normalizeSeparators)
    : normalizeSeparators(glob);
...
export function globSync(p: FilePath, fs: FileSystem, options?: FastGlobOptions<FilePath>): Array<FilePath> {
  options = {
    ...options,
    fs: {
      statSync: (p) => {
...
export function glob(p: FilePath, fs: FileSystem, options: FastGlobOptions<FilePath>): Promise<Array<FilePath>> {
  options = {
    ...options,
    fs: {
      stat: async (p, cb) => {
