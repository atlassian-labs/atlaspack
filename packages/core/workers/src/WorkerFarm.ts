...
export default class WorkerFarm extends EventEmitter {
  ...
  serializedSharedReferences: Map<SharedReference, ArrayBuffer | null | undefined> = new Map();
  ...
  workerApi: {
    ...
    callMaster: async (request: CallRequest, awaitResponse: boolean | null = true): Promise<unknown> => {
    ...
    createReverseHandle: (fn: HandleFunction): Handle => this.createReverseHandle(fn),
    callChild: (childId: number, request: HandleCallRequest): Promise<unknown> => new Promise((resolve: (result: Promise<any> | any) => void, reject: (error?: any) => void) => {
    ...
    runHandle: (handle: Handle, args: Array<any>): Promise<unknown> => this.workerApi.callChild(nullthrows(handle.childId), {
      handle: handle.id,
      args,
    }),
  ...
  createSharedReference(value: unknown, isCacheable: boolean = true): {
    ref: SharedReference,
    ...
    return {
      ...
      dispose: () => {
        ...
        for (let worker of this.workers.values()) {
          ...
          promises.push(
            new Promise((resolve: (result: Promise<any> | any) => void, reject: (error?: any) => void) => {
              worker.call({
  ...
  static getConcurrentCallsPerWorker(defaultValue: number = DEFAULT_MAX_CONCURRENT_CALLS): number {
    return (
