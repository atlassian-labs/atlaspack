...
// import {WorkerFarm} from '../index.mts';
...
import url from 'node:url';
import {WorkerFarm} from '../index.mts';
import type { Serializable, Transferrable } from '../worker-interface.mts';
...
describe('WorkerFarm', function () {
  ...
  it('Should start up workers', async () => {
    let workerfarm = new WorkerFarm({
      workerPath: path.join(__dirname, 'integration', 'ping.cjs'),
    });
  ...
  it('Should handle 1000 requests without any issue', async () => {
    let workerfarm = new WorkerFarm({
      workerPath: path.join(__dirname, 'integration', 'echo.cjs'),
    });
  ...
  it('Should warm up workers', async () => {
    ...
    await workerfarm.onReady();

    const status = workerfarm.status();
    assert(status.workers.length > 0, 'Should have spawned workers.');
    await workerfarm.end();
  });

  it('Should be able to use bi-directional communication', async () => {
    let workerfarm = new WorkerFarm({
      workerPath: path.join(__dirname, 'integration', 'ipc.cjs'),
    });
