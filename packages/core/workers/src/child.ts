...
export class Child {
  ...
  workerApi: {
    ...
    callMaster: (request: CallRequest, awaitResponse: boolean | null = true): Promise<unknown> => this.addCall(request, awaitResponse),
    createReverseHandle: (fn: (...args: Array<any>) => unknown): Handle => this.createReverseHandle(fn),
    runHandle: (handle: Handle, args: Array<any>): Promise<unknown> => this.workerApi.callMaster({handle: handle.id, args}, true),
  ...
  addCall(request: CallRequest, awaitResponse: boolean | null = true): Promise<unknown> {
    let call: ChildCall = {
      ...request,
      type: 'request',
      child: this.childId,
    ...
    let promise;
    if (awaitResponse) {
      promise = new Promise((resolve: (result: Promise<any> | any) => void, reject: (error?: any) => void) => {
