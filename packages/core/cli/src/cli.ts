...
const handleUncaughtException = async (exception: any) => {
  try {
...
function parseOptionInt(value: any) {
...
function shouldUseProductionDefaults(command: any) {
  return command.name() === 'build' || command.production === true;
}

async function normalizeOptions(command: any, inputFS: NodeFS): Promise<InitialAtlaspackOptions> {
  let nodeEnv;
