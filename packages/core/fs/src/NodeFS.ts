...
export class NodeFS implements FileSystem {
  ...
  createReadStream: (path: string, options?: any) => ReadStream = fs.createReadStream;
  cwd: () => string = () => process.cwd();
  chdir: (directory: string) => void = (directory) => process.chdir(directory);

  statSync: (path: string) => Stats = (path) => fs.statSync(path);
  realpathSync: (path: string, cache?: any) => string = process.platform === 'win32' ? fs.realpathSync : fs.realpathSync.native;
