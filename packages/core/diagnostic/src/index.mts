...
// TSMIGRATE: Might return a Diagnostic | Array<Diagnostic>
...
export default class ThrowableDiagnostic extends Error {
  ...
  constructor(opts: ThrowableDiagnosticOpts) {
    const diagnostics = Array.isArray(opts.diagnostic)
      ? opts.diagnostic
      : [opts.diagnostic];
...
export function generateJSONCodeHighlights(
  ...
  return ids.map(({key, type, message}) => {
    const pos = nullthrows.default(map.pointers[key]);
    return {
...
export function getJSONHighlightLocation(
  ...
  const key = 'key' in pos ? pos.key : undefined;
  const keyEnd = 'keyEnd' in pos ? pos.keyEnd : undefined;
...
export function getJSONSourceLocation(
  ...
  const v = getJSONHighlightLocation(pos, type);
