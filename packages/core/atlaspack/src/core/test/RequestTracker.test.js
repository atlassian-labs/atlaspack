...
import {FILE_CREATE, FILE_UPDATE, INITIAL_BUILD} from '../constants';
import {makeDeferredWithPromise} from '../../utils/index.js';
import {toProjectPath} from '../projectPath';
import {DEFAULT_FEATURE_FLAGS, setFeatureFlags} from '../../feature-flags/index.js';

const options = DEFAULT_OPTIONS;
const farm = new WorkerFarm({workerPath: require.resolve('../worker')});
            ...
            run: async ({api}: {api: RunAPI<string | void>, ...}) => {
              api.invalidateOnFileCreate({
                ...
                aboveFilePath: toProjectPath(
                  '',
                  './node_modules/somethi/package.json',
                ),
              });
              let result = await Promise.resolve('hello');
              api.storeResult(result);
