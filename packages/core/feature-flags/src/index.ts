...
export function getFeatureFlagValue(flagName: keyof FeatureFlags): boolean | string | number {
...
export function runWithConsistencyCheck<Result, CustomDiagnostic>(
  ...
  oldFn: () => Result,
  newFn: () => Result,
  diffFn: (oldResult: Result, newResult: Result) => DiffResult<CustomDiagnostic>,
  report: (
