...
export function createAssetGraphRequestRust(rustAtlaspack: AtlaspackV3): (input: AssetGraphRequestInput) => AssetGraphRequest {
  return (input: AssetGraphRequestInput) => ({
    ...
    run: async (input) => {
      ...
      await Promise.all(
        Array.from(cachedAssets.entries(), ([id, code]: [any, any]) =>
...
function getAssetGraph(serializedGraph: any, options: AtlaspackOptions) {
