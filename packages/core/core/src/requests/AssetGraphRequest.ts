...
export class AssetGraphBuilder {
  ...
  queueCorrespondingRequest(nodeId: NodeId, errors: Array<Error>): Promise<unknown> {
    let promise;
    let node = nullthrows(this.assetGraph.getNode(nodeId));
    switch (node.type) {
  ...
  async runEntryRequest(input: ProjectPath) {
    ...
    let result = await this.api.runRequest<ProjectPath, EntryRequestResult>(request, {
      force: true,
    });
  ...
  async runPathRequest(input: Dependency) {
    ...
    let result = await this.api.runRequest<PathRequestInput, AssetGroup | null | undefined>(request, {force: true});
  ...
  async runAssetRequest(input: AssetGroup) {
    ...
    let assets = await this.api.runRequest<AssetRequestInput, Array<Asset>>(request, {force: true});
