...
export function getCachedAtlaspackConfig(result: ConfigAndCachePath, options: AtlaspackOptions): AtlaspackConfig {
...
export async function loadAtlaspackConfig(options: AtlaspackOptions): Promise<(AtlaspackConfigChain) & {
  usedDefault: boolean
}> {
...
export async function resolveAtlaspackConfig(options: AtlaspackOptions): Promise<(AtlaspackConfigChain) & {
  usedDefault: boolean
} | null | undefined> {
  let resolveFrom = getResolveFrom(options.inputFS, options.projectRoot);
  let configPath =
...
export function create(config: ResolvedAtlaspackConfigFile, options: AtlaspackOptions): Promise<AtlaspackConfigChain> {
  return processConfigChain(config, config.filePath, options);
}

// eslint-disable-next-line require-await
export async function parseAndProcessConfig(configPath: FilePath, contents: string, options: AtlaspackOptions): Promise<AtlaspackConfigChain> {
...
export async function processConfig(configFile: ResolvedAtlaspackConfigFile, options: AtlaspackOptions): Promise<ProcessedAtlaspackConfig> {
  return {
...
export function mergeConfigs(base: ProcessedAtlaspackConfig, ext: ProcessedAtlaspackConfig): ProcessedAtlaspackConfig {
  return {
...
export function getResolveFrom(fs: FileSystem, projectRoot: FilePath): FilePath {
  let cwd = fs.cwd();
  let dir = isDirectoryInside(cwd, projectRoot) ? cwd : projectRoot;
  return path.join(dir, 'index');
}

function assertPurePipeline(pipeline: ExtendableAtlaspackConfigPipeline): PureAtlaspackConfigPipeline {
