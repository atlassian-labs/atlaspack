...
async function run({input, api, options}) {
  ...
  let configResult = nullthrows(
    await api.runRequest<null, ConfigAndCachePath>(createAtlaspackConfigRequest()),
  );
...
export class TargetResolver {
  ...
  async resolve(rootDir: FilePath, exclusiveTarget?: string): Promise<Array<Target>> {
  ...
  async resolvePackageTargets(rootDir: FilePath, exclusiveTarget?: string): Promise<Map<string, Target | null>> {
    ...
    let getEnginesLoc = (targetName: string, descriptor: PackageTargetDescriptor): TargetKeyInfo => {
...
function assertNoDuplicateTargets(options: AtlaspackOptions, targets: Map<string, Target | null>, pkgFilePath: FilePath | null | undefined, pkgContents: undefined | string) {
...
function normalizeSourceMap(options: AtlaspackOptions, sourceMap: undefined | TargetSourceMapOptions | boolean) {
...
async function debugResolvedTargets(input: Entry, targets: TargetRequestResult, targetInfo: Map<string, TargetInfo>, options: AtlaspackOptions) {
  for (let target of targets) {
    ...
    let highlightsWithMessages = [...jsonHighlights].map(([k, v]: [any, any]) => {
    ...
    let format = (v: TargetKeyInfo) => (v.message != null ? md.italic(v.message) : '');
    logger.verbose({
      origin: '@atlaspack/core',
