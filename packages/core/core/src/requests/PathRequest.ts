...
export default function createPathRequest(input: PathRequestInput): PathRequest {
  return {
...
export class ResolverRunner {
  ...
  async getDiagnostic(dependency: Dependency, message: string): Promise<Async<Diagnostic>> {
  ...
  async loadConfigs(resolvers: Array<LoadedPlugin<Resolver<unknown>>>): Promise<void> {
