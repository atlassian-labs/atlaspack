...
import type {Bundle, AtlaspackOptions, ProcessedAtlaspackConfig, RequestInvalidation} from './types';
...
type WorkerTransformationOpts = (Flow.Diff<TransformationOpts, {
  workerApi: unknown,
...
type WorkerValidationOpts = (Flow.Diff<ValidationOpts, {
  workerApi: unknown,
  options: AtlaspackOptions
}>) & {
  optionsRef: SharedReference,
...
export async function runTransform(workerApi: WorkerApi, opts: WorkerTransformationOpts): Promise<TransformationResult> {
...
export async function runValidate(workerApi: WorkerApi, opts: WorkerValidationOpts): Promise<void> {
  let {optionsRef, configCachePath, ...rest} = opts;
  let options = loadOptions(optionsRef, workerApi);
  let config = await loadConfig(configCachePath, options);
