...
let valueToMutableDependencySymbols: WeakMap<Dependency, MutableDependencySymbols> = new WeakMap();
  ...
  */
  ...
  [inspect]() {
    return `MutableDependencySymbols(${
      this.#value.symbols
        ? [...this.#value.symbols]
            .map(([s, {local, isWeak}]: [any, any]) => `${s}:${local}${isWeak ? '?' : ''}`)
            .join(', ')
        : null
    })`;
  }
...
function fromInternalAssetSymbol(projectRoot: string, value: undefined | {
...
function fromInternalDependencySymbol(projectRoot: string, value: undefined | {
  isWeak: boolean,
  loc: InternalSourceLocation | null | undefined,
  local: symbol,
  meta?: Meta | null | undefined
