...
export function createAsset(projectRoot: FilePath, options: AssetOptions): Asset {
  return {
    ...
    symbols:
      options.symbols &&
      new Map(
        [...options.symbols].map(([k, v]: [any, any]) => [
          k,
          {
            local: v.local,
            meta: v.meta,
...
export function generateFromAST(asset: CommittedAsset | UncommittedAsset): Promise<GenerateOutput> {
...
async function _generateFromAST(asset: CommittedAsset | UncommittedAsset) {
  ...
  let {plugin} = await loadPlugin<Transformer<unknown>>(pluginName, fromProjectPath(
    asset.options.projectRoot,
    nullthrows(asset.value.configPath),
  ), nullthrows(asset.value.configKeyPath), asset.options);
...
export async function getInvalidationHash(invalidations: Array<RequestInvalidation>, options: AtlaspackOptions): Promise<string> {
