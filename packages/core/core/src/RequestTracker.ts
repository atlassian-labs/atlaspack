...
const nodeFromConfigKey = (fileName: ProjectPath, configKey: string[], contentHash: string): RequestGraphNode => ({
...
const keyFromEnvContentKey = (contentKey: ContentKey): string => contentKey.slice('env:'.length);

const keyFromOptionContentKey = (contentKey: ContentKey): string => contentKey.slice('option:'.length);
...
export class RequestGraph extends ContentGraph<RequestGraphNode, RequestGraphEdgeType> {
  ...
  async respondToFSEvents(
    ...
    const invalidateNode = (nodeId: NodeId, reason: InvalidateReason | number) => {
...
export default class RequestTracker {
  ...
  async getRequestResult<T extends RequestResult>(contentKey: ContentKey, ifMatch?: string): Promise<T | null | undefined> {
  ...
  async runRequest<TInput, TResult extends RequestResult>(request: Request<TInput, TResult>, opts?: RunRequestOpts | null): Promise<TResult> {
  ...
  createAPI<TResult extends RequestResult>(requestId: NodeId, previousInvalidations: Array<RequestInvalidation>): {
    ...
    let api: RunAPI<TResult> = {
      ...
      getRequestResult: <T extends RequestResult>(id): Async<T | null | undefined> => this.getRequestResult<T>(id),
  ...
  async writeToCache(signal?: AbortSignal) {
    ...
    async function runCacheImprovements<T>(newPath: (cache: LMDBLiteCache) => Promise<T>, oldPath: () => Promise<T>): Promise<T> {
      if (getFeatureFlag('cachePerformanceImprovements')) {
    ...
    try {
      ...
      const serialiseAndSet = async (key: string, contents: any): Promise<void> => {
        if (signal?.aborted) {
...
export async function readAndDeserializeRequestGraph(cache: Cache, requestGraphKey: string, cacheKey: string): Promise<Async<{
...
async function loadRequestGraph(options: AtlaspackOptions): Promise<Async<RequestGraph>> {
 ...
 */
export async function invalidateRequestGraph(requestGraph: RequestGraph, options: AtlaspackOptions, events: Event[]): Promise<InvalidationStats> {
 ...
 */
export async function runInvalidation(requestGraph: RunInvalidationInput, invalidationFn: InvalidationFn): Promise<InvalidationFnStats> {
...
function logErrorOnBailout(options: AtlaspackOptions, snapshotPath: string, e: Error): void {
...
export function cleanUpOrphans<N, E extends number>(graph: Graph<N, E>): NodeId[] {
 ...
 */
export function getBiggestFSEventsInvalidations(invalidationsByPath: Map<string, number>, limit: number = 10): Array<FSInvalidation> {
