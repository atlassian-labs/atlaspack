...
export default class PackagerRunner {
  ...
  async loadConfigs(bundleGraph: InternalBundleGraph, bundle: InternalBundle): Promise<{
  ...
  async loadPluginConfig<T extends PluginWithBundleConfig>(
    ...
    if (!bundleConfigs.has(plugin.name) && loadBundleConfig != null) {
      ...
      config.result = await loadBundleConfig({
        ...
        bundleGraph: new BundleGraph<NamedBundleType>(bundleGraph, NamedBundle.get.bind(NamedBundle), this.options),
  ...
  async optimize(
    ...
    let bundleGraph = new BundleGraph<NamedBundleType>(internalBundleGraph, NamedBundle.get.bind(NamedBundle), this.options);
  ...
  async generateSourceMap(bundle: InternalBundle, map: SourceMap): Promise<string> {
  ...
  async writeToCache(cacheKeys: CacheKeyMap, type: string, contents: Blob, map?: string | null): Promise<BundleInfo> {
    let size = 0;
    let hash;
