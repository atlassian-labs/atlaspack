...
export default class Atlaspack {
  ...
  #watchQueue /*: PromiseQueue<?BuildEvent>*/ = new PromiseQueue<BuildEvent | null | undefined>({
    maxConcurrent: 1,
  });
  ...
  async watch(cb?: (err?: Error | null | undefined, buildEvent?: BuildEvent) => unknown): Promise<AsyncSubscription> {
    if (!this.#initialized) {
      await this._init();
    }
   ...
   */
  async unstable_buildAssetGraph(writeToCache: boolean = true): Promise<AssetGraphRequestResult> {
    await this._init();
  ...
  async unstable_transform(options: AtlaspackTransformOptions): Promise<Array<Asset>> {
    if (!this.#initialized) {
      await this._init();
    }
  ...
  async unstable_resolve(request: AtlaspackResolveOptions): Promise<AtlaspackResolveResult | null | undefined> {
    if (!this.#initialized) {
      await this._init();
    }
...
export function createWorkerFarm(options: Partial<FarmOptions> = {}): WorkerFarm {
