...
function nullthrowsAssetNode(v?: AssetGraphNode | null): AssetNode {
  invariant(v?.type === 'asset');
  return v;
}
function nullthrowsDependencyNode(v?: AssetGraphNode | null): DependencyNode {
...
function assertUsedSymbols(
  ...
  let expectedAsset = new Map(
    _expectedAsset.map(([f, symbols]: [any, any]) => [f, symbols]),
  );
  let expectedDependency = new Map(
    _expectedDependency.map(([from, to, sym]: [any, any, any]) => [
      from + ':' + to,
      sym ? sym.map((v: any) => [v[0], v[1] ?? [to, v[0]]]) : sym,
    ]),
  );
  ...
  if (isLibrary) {
    let entryDep = nullthrows(
      [...graph.nodes.values()].find(
        (n: any) => n?.type === 'dependency' && n.value.sourceAssetId == null,
      ),
    );
...
function changeDependency(
  ...
  from: FilePath,
  to: FilePath,
  cb: (arg1: NonNullable<Dependency['symbols']>) => void,
): Iterable<[ContentKey, Asset]> {
  let sourceAssetNode = nullthrowsAssetNode(
    [...graph.nodes.values()].find(
      (n: any) => n?.type === 'asset' && n.value.filePath === from,
    ),
  );
...
function changeAsset(
  graph: AssetGraph,
  asset: FilePath,
  cb: (arg1: NonNullable<Asset['symbols']>) => void,
): Iterable<[ContentKey, Asset]> {
  let node = nullthrowsAssetNode(
    [...graph.nodes.values()].find(
      (n: any) => n?.type === 'asset' && n.value.filePath === asset,
    ),
  );
...
describe('SymbolPropagation', () => {
  ...
  it('basic tree - dependency symbol change export', async () => {
    ...
    let changedAssets = [
      ...changeDependency(graph, 'index.js', '/lib.js', (symbols: any) => {
        symbols.set('b', {
          local: 'b',
          isWeak: false,
          loc: undefined,
  ...
  it('basic tree - asset symbol change export and error', async () => {
    ...
    let changedAssets = [
      ...changeAsset(graph, 'lib.js', (symbols: any) => {
        symbols.delete('f');
        symbols.set('f2', {
          local: 'f2',
          loc: undefined,
