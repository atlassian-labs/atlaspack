...
describe('javascript', function () {
  ...
  it('does not fail when there is a transitive import of an empty file with export *', async () => {
  ...
  it.v2(
    ...
    async function () {
      let logs: Array<
        ProgressLogEvent | DiagnosticLogEvent | TextLogEvent | any
      > = [];
  ...
  it.skip('should support adding implicit dependencies', async function () {
    let b = await bundle(path.join(__dirname, '/integration/json/index.js'), {
      delegate: {
        getImplicitDependencies(asset: any) {
  ...
  it('should expose to RequireJS entry point', async function () {
    ...
    let test;
    const mockDefine = function (f: any) {
      test = f();
    };
    mockDefine.amd = true;
  ...
  it.skip('should not dedupe imports with different contents', async function () {
    let b = await bundle(
      ...
      {
        // enable asset dedupe in JSPackager
        hmr: false,
      },
    );
  ...
  it('can static import and dynamic import in the same bundle when another bundle requires async', async () => {
    ...
    assert.deepEqual(await runSameBundleResult.default, [42, 42, 42]);
  ...
  it('can run an entry bundle whose entry asset is present in another bundle', async () => {
    ...
    assert.equal(await runJsBundle.default, 43);
  });
  ...
  describe('tokens', () => {
    it('should transform tokens', async () => {
      ...
      assert(
        ...
        `Expected var(--ds-text, #172B4D) to be in the bundle, but bundle was ${firstBundle}`,
      );
    });

    it('should fail to transform tokens if file is invalid', async () => {
      await fsFixture(overlayFS, __dirname)`
        tokens
          index.js:
            import { token } from '@atlaskit/tokens';
          ...
          package.json:
            {
              ...
              "@atlaspack/transformer-js": {
                "atlaskitTokens": {
                  "tokenDataPath": "/zUsers/mszczepanski/Work/atlassian/afm/pillar-jira-ssr/platform/packages/design-system/tokens/src/artifacts/token-data.json"
                }
              }
            }
          yarn.lock: {}
      ...
      const b = await bundler(path.join(__dirname, 'tokens/index.js'), {
        inputFS: overlayFS,
        outputFS: overlayFS,
        mode: 'production',
      });
      try {
        ...
        assert(err.diagnostics.length > 0);
        assert(err.diagnostics[0].message.includes('Failed to load token map'));
      }
    });
  });
});
