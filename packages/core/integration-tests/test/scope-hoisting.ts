...
[false, true].forEach((exportsRebindingOptimisation) => {
  describe(`scope hoisting with exports rebinding optimisation: ${exportsRebindingOptimisation}`, function () {
    ...
    const bundler = (name: string, opts = {}) => {
      return _bundler(
        name,
        mergeParcelOptions(
          {
    ...
    describe('es6', function () {
      ...
      it('should handle TSC polyfills', async () => {
        ...
        let b = await bundle(
          path.join(__dirname, 'tsc-polyfill-es6/index.js'),
          {
            inputFS: overlayFS,
          },
        );
      ...
      it.v2(
        'should error when assigning to a default import',
        async function () {
          let source = path.join(
            __dirname,
    ...
    describe('commonjs', function () {
      ...
      it.v2('should support two aliases to the same module', async function () {
        let b = await bundle(
          path.join(
            __dirname,
    ...
    it.v2(
      'unmark an asset group as deferred when it becomes used',
      async function () {
        let testDir = path.join(
          __dirname,
    ...
    it('produce the same bundle hash regardless of transformation order', async function () {
      ...
      const waitHandler = (fileToDelay: string, fileToWaitFor: string) => {
        ...
        return {
          get(target: OverlayFS, prop: string) {
    ...
    it('should add experimental bundle queue runtime to manual shared bundles', async function () {
      ...
      function hasAsset(bundle: PackagedBundle, assetName: string) {
        let result = false;
    ...
    describe('scope hoisting within wrapped assets', function () {
      function assertWrappedAssets(
        ...
        assert.equal(
          bundleCode.match(/parcelRegister\(/g).length,
          assets.length,
        );
      ...
      `;
            ...
            let b = await bundle(
              path.join(
                __dirname,
                'star-reexports-in-wrapped-groups/index.html',
              ),
              {
                inputFS: overlayFS,
                featureFlags: {
    ...
    describe('exports rebinding optimisation', function () {
      ...
      `;
        ...
        assert.ok(
          sharedBundleContents.includes(
            '$parcel$export(module.exports, "one",',
          ),
