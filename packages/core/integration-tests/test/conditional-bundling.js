...
describe('conditional bundling', function () {
  ...
  it(`should have no duplicated deps as bundles in conditional manifest`, async function () {
    const dir = path.join(__dirname, 'import-cond-cond-manifest-no-duplicates');
    await overlayFS.mkdirp(dir);
    ...
    await fsFixture(overlayFS, dir)`
        ...
        index.js:
          const imported1 = importCond('cond', './a', './b');
          const imported2 = importCond('cond', './c', './d');
          const imported3 = importCond('cond', './c', './d');
