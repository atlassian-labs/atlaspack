...
describe('fs', function () {
  ...
  describe('browser environment', function () {
    ...
    // This is not ideal configuration, and should not be supported in v3 as there is already a
    // transformer config option that is sufficient for disabling this behaviour
    it.v2(
      'should not evaluate fs calls when package.browser.fs is false',
      async function () {
        let b = await bundle(
