...
const config = path.join(__dirname, '../data/integration/typescript-config/.parcelrc');

describe.v2('typescript tsc', function () {
  it('should support loading tsconfig.json', async () => {
    let b = await bundle(
      path.join(__dirname, '../data/integration/typescript-config/index.ts'),
      {config},
    );
  ...
  it('should support loading tsconfig.json with extends', async () => {
    let b = await bundle(
      path.join(__dirname, '../data/integration/typescript-config-extends/index.ts'),
      {config},
    );
  ...
  it('should produce a type declaration file when overriding the ts pipeline', async function () {
    let b = await bundle(
      path.join(__dirname, '../data/integration/typescript-types-parcelrc/index.ts'),
    );
