...
describe('inline requires', () => {
  [true, false].forEach((inlineRequiresMultiThreading) => {
    setFeatureFlags({
      ...DEFAULT_FEATURE_FLAGS,
      inlineRequiresMultiThreading,
    });

    describe(
      inlineRequiresMultiThreading ? 'multi-threaded' : 'single-threaded',
      () => {
        let options = {
    ...
    `;
          ...
          const line =
            bundleContents
              .split('\n')
              .findIndex((line) => line.includes('line 7')) + 1;
