import type {InitialAtlaspackOptions, BuildSuccessEvent} from '@atlaspack/types';
...
[false, true].forEach((cachePerformanceImprovements) => {
  ...
  function runBundle(entries = 'src/index.js', opts, featureFlags: undefined | Partial<FeatureFlags>) {
    return bundler(getEntries(entries), getOptions(opts, featureFlags)).run();
  }

  type UpdateFn = (arg1: BuildSuccessEvent) => InitialAtlaspackOptions | null | undefined | Promise<InitialAtlaspackOptions | null | undefined>;
  ...
  async function testCache(
    ...
    let entries;
    let options: InitialAtlaspackOptions | null | undefined;
  ...
  describe.v2(
    ...
    function () {
      ...
      it('should support adding a dependency which changes the referenced bundles of a parent bundle', async function () {
        async function exec(bundleGraph: BundleGraph<PackagedBundle>, bundle: PackagedBundle) {
          let calls: Array<any> = [];
            ...
            let b = await testCache({
              ...
              async setup() {
                ...
                await inputFS.writeFile(
                  path.join(
                    inputDir,
                    'packages/babel-plugin-dummy/package.json',
                  ),
                  JSON.stringify({
                    name: 'babel-plugin-dummy',
                    version: '1.0.0',
        ...
        it('should update when publicUrl changes', async function () {
          ...
          let b = await testCache({
            ...
            async update(b: BuildSuccessEvent) {
              ...
              assert(
                contents.includes(
                  '<script type="module" src="http://example.com',
                ),
              ...
              let b = await testCache(
                {
                  ...
                  async setup() {
                    ...
                    await inputFS.mkdirp(
                      path.join(inputDir, 'pnp/testmodule2'),
                    );
                    await inputFS.writeFile(
            ...
            try {
              b = await testCache({
                ...
                async update(b: BuildSuccessEvent) {
                  ...
                  await new Promise((resolve: (result: Promise<undefined> | undefined) => void) => setTimeout(resolve, 20));
                  return {
                    workerFarm,
                  };
                },
      ...
      describe('packaging', function () {
        ...
        it('should invalidate when deleting a source map', async function () {
          await testCache({
            ...
            async update() {
              assert(
                await overlayFS.exists(path.join(distDir, 'index.js.map')),
              );
        ...
        it('should react to changes in tsconfig.json compilerOptions', async () => {
          ...
          const b = await testCache({
            ...
            async update(b: BuildSuccessEvent) {
              ...
              await new Promise((resolve: (result: Promise<undefined> | undefined) => void) => setTimeout(resolve, 1000));
            },
          });
  ...
  color: red;
      ...
      it('properly watches included files after a transformer error', async function () {
        ...
        try {
          ...
          // Change included file
          await overlayFS.writeFile(
            path.join(fixture, 'included.txt'),
            'ERROR',
          );
          event = await getNextBuild(b);
