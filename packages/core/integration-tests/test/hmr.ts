...
async function openSocket(uri: string, opts: any) {
  ...
  await new Promise((resolve: (result: Promise<never>) => void, reject: (error?: any) => void) => {
...
async function nextWSMessage(ws: WebSocket) {
  return json5.parse(
    await new Promise((resolve: (result: Promise<never>) => void) => ws.once('message', resolve)),
  );
}
...
describe.v2('hmr', function () {
  ...
  async function testHMRClient(
    name: string,
    updates: ((arg1: Array<any>) => {
      [key: string]: string;
    }) | Array<(arg1: Array<any>) => {
      [key: string]: string;
    }>,
  ) {
    await ncp(
