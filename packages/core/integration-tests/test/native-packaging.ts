...
async function runSsrPackagingTest() {
  ...
  const bundle2 = b.getBundles().find((bundle) => bundle.name === 'index2.js');
...
describe('Native packaging', function () {
  ...
  it('should package two SSR entries', function () {
    return runSsrPackagingTest();
  });

  describe.v3('full native build', function () {
    setupV3Flags({
      fullNative: true,
    });
