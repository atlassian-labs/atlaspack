...
describe.v2('glsl', function () {
  it('should support requiring GLSL files via glslify', async function () {
    let b = await bundle(path.join(__dirname, '../data/integration/glsl/index.js'));

    let shader = fs.readFileSync(
      path.join(__dirname, '../data/integration/glsl/compiled.glsl'),
      'utf8',
    );
  ...
  it('should correctly resolve relative GLSL imports', async function () {
    let b = await bundle(
      path.join(__dirname, '../data/integration/glsl-relative-import/index.js'),
    );
