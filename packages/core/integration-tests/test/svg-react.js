...
describe.v2('svg-react', function () {
  it('should support transforming SVGs to react components', async function () {
    let b = await bundle(
      path.join(__dirname, '../data/integration/svg-react/react.js'),
      {
        defaultConfig: path.join(
          __dirname,
          '../data/integration/custom-configs/.parcelrc-svg-react',
        ),
      },
    );
  ...
  it('should support transforming SVGs to typescript react components', async function () {
    let b = await bundle(
      path.join(__dirname, '../data/integration/svg-react-typescript/react.ts'),
      {
        defaultConfig: path.join(
          __dirname,
          '../data/integration/custom-configs/.parcelrc-svg-react',
  ...
  it('should find and use a .svgrrc and .svgorc config file', async function () {
    let b = await bundle(
      path.join(__dirname, '../data/integration/svg-react-config/react.js'),
      {
        defaultConfig: path.join(
          __dirname,
          '../data/integration/custom-configs/.parcelrc-svg-react',
