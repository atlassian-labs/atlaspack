...
export default class AdjacencyList<TEdgeType extends number = NullEdgeType> {
   ...
   */
  constructor(
    opts?: SerializedAdjacencyList<TEdgeType | NullEdgeType> | AdjacencyListOptions<TEdgeType | NullEdgeType>,
  ) {
    let nodes;
    let edges;
   ...
   */
  addEdge(from: NodeId, to: NodeId, type: TEdgeType | NullEdgeType = NULL_EDGE_TYPE): boolean {
  ...
  *getAllEdges(): Iterator<{
  ...
  hasEdge(
    from: NodeId,
    to: NodeId,
    type: TEdgeType | NullEdgeType | Array<TEdgeType | NullEdgeType> = NULL_EDGE_TYPE,
  ): boolean {
   ...
   */
  removeEdge(from: NodeId, to: NodeId, type: TEdgeType | NullEdgeType = NULL_EDGE_TYPE): void {
    let hash = this.#edges.hash(from, to, type);
  ...
  getNodeIdsConnectedFrom(
    from: NodeId,
    type: AllEdgeTypes | TEdgeType | NullEdgeType | Array<TEdgeType | NullEdgeType> = NULL_EDGE_TYPE,
  ): NodeId[] {
 ...
 *             └─────────────────────────────────────────────┘
 */
export class SharedTypeMap<TItemType, THash, TAddress extends number> implements Iterable<TAddress> {
  /**
 ...
 */
export class NodeTypeMap<TEdgeType> extends SharedTypeMap<TEdgeType, NodeId, NodeAddress> {
  /**
 ...
 * the `from` node.
 */
export class EdgeTypeMap<TEdgeType> extends SharedTypeMap<TEdgeType, EdgeHash, EdgeAddress> {
  /**
   * In addition to the header defined by `SharedTypeMap`, the header for
...
function increaseNodeCapacity(currentCapacity: number, params: AdjacencyListParams): number {
...
function increaseEdgeCapacity(currentCapacity: number, params: AdjacencyListParams): number {
...
function decreaseEdgeCapacity(currentCapacity: number, params: AdjacencyListParams): number {
  return Math.max(
