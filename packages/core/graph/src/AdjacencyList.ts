...
import {ALL_EDGE_TYPES, NullEdgeType, AllEdgeTypes} from './Graph';
...
export default class AdjacencyList<TEdgeType extends number = 1> {
   ...
   */
  static deserialize<TEdgeType extends number = 1>(opts: SerializedAdjacencyList<TEdgeType>): AdjacencyList<TEdgeType> {
  ...
  *getAllEdges(): Iterator<{
   ...
   */
  removeEdge(from: NodeId, to: NodeId, type: TEdgeType | NullEdgeType = 1): void {
    let hash = this.#edges.hash(from, to, type);
  ...
  getNodeIdsConnectedFrom(
    from: NodeId,
    type: AllEdgeTypes | TEdgeType | NullEdgeType | Array<TEdgeType | NullEdgeType> = 1,
  ): NodeId[] {
