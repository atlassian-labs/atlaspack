 ...
 */
type DFSCommand<TContext> = DFSCommandVisit<TContext> | DFSCommandExit<TContext>;
...
export default class Graph<TNode, TEdgeType extends number = NullEdgeType> {
  ...
  constructor(opts?: GraphOpts<TNode, TEdgeType> | null) {
    ...
    this.adjacencyList = adjacencyList
      ...
      : new AdjacencyList<TEdgeType>(typeof initialCapacity === 'number' ? {initialCapacity} : undefined);
  }
  ...
  getNodeIdsConnectedTo(
    nodeId: NodeId,
    type: TEdgeType | NullEdgeType | Array<TEdgeType | NullEdgeType> | AllEdgeTypes = NULL_EDGE_TYPE,
  ): Array<NodeId> {
...
export function mapVisitor<NodeId, TValue, TContext>(
  ...
  function makeEnter(visit: (
    ...
    return function mappedEnter(nodeId: NodeId, context: TContext | null | undefined, actions: TraversalActions) {
  ...
  if (visit.exit != null) {
    mapped.exit = function mappedExit(nodeId: NodeId, context: TContext | null | undefined, actions: TraversalActions) {
      let exit = visit.exit;
      if (!exit) {
        return;
      }
