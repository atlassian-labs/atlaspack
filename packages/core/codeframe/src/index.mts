...
export default function codeFrame(
  ...
  const lineNumberPrefixer = (params: {
    ...
    const {lineNumber, lineNumberLength, isHighlighted} = params;
  ...
  if (endLineIndex - startLine > opts.maxLines) {
    const maxLine = startLine + opts.maxLines - 1;
  ...
  const lineNumberLength = (endLineIndex + 1).toString(10).length;

  // Split input into lines and highlight syntax
  const lines = code.split(NEWLINE);
  const syntaxHighlightedLines = (
  ...
  for (
    ...
    const lineWidth = stringWidth(syntaxHighlightedLine);
    let highlightLine = '';
    if (isWholeLine) {
