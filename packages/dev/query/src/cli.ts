...
export async function run(input: string[]) {
  ...
  function _getIncomingNodeOfType(bundleGraph: any, node: any, type: string) {
    if (!hasBundleGraph()) {
      return;
    }
    invariant(bundleGraph != null);
  ...
  function _getBundlePriority(bundleGraph: any, bundle: any) {
    if (!hasBundleGraph()) {
      return;
    }
    invariant(bundleGraph != null);
  ...
  function _findEntryBundle(bundleGraph: any, node: any) {
    if (!hasBundleGraph()) {
      return;
    }
    invariant(bundleGraph != null);
  ...
  function timeSerialize(graph: any | null | undefined) {
    let date = Date.now();
    serialize(graph);
    date = Date.now() - date;
    return date;
