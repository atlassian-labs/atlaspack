...
export async function run(input: string[]) {
  ...
  function _getIncomingNodeOfType(bundleGraph: BundleGraph, node, type: string) {
    if (!hasBundleGraph()) {
      return;
    }
    invariant(bundleGraph != null);
  ...
  function timeSerialize(graph: AssetGraph | null | undefined | BundleGraph | RequestTracker) {
    let date = Date.now();
    serialize(graph);
    date = Date.now() - date;
    return date;
  }
  function _printStatsTable(header: string, data: Array<Array<string | number>> | Array<[string, unknown]>) {
    const config = {
      columnDefault: {
        width: 18,
      },
