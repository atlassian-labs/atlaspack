...
export function ArrowRenderer({
  ...
  useEffect(() => {
    const updatePaths = () => {
      ...
      const newPaths: {id: string; d: string}[] = connections
        .map(({from, to}) => {
          ...
          const startY =
            fromRect.top - containerRect.top + scrollTop + fromRect.height / 2;
          const endX = toRect.left - containerRect.left + scrollLeft;
          const endY =
            toRect.top - containerRect.top + scrollTop + toRect.height / 2;
