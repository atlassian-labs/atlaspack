...
export class FS implements Iterable<[string, File | FSMap]> {
  ...
  get(path: string): File | null | undefined {
  ...
  list(files: FSMap = this.files, prefix: string = ''): Map<string, File> {
    let result: Array<Array<File | string> | [string, File]> = [];
...
const HMR_OPTIONS: Partial<REPLOptions> = {
  mode: 'development',
  hmr: true,
  scopeHoist: false,
  sourceMaps: true,
...
export const ASSET_PRESETS: Map<string, {
