...
export async function downloadZIP(files: Map<string, {
  value: string
}>) {
  let zip = new JSZip();
...
export async function extractZIP(content: ArrayBuffer): Promise<FSMap> {
  ...
  let files = (
    await Promise.all(
      Object.entries(zip.files).map(async ([relativePath, zipEntry]: [any, any]) => {
        if (!zipEntry.dir) {
...
export function linkSourceMapVisualization(bundle: string, sourcemap: string): string {
