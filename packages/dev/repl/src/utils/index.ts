...
export async function extractZIP(content: ArrayBuffer): Promise<FSMap> {
  ...
  let files = (
    await Promise.all(
      Object.entries(zip.files).map(
        async ([relativePath, zipEntry]: [any, any]) => {
          if (!zipEntry.dir) {
  ...
  function get(p: Array<string>): FSMap {
    let v = result;
    for (let e of p) {
      let c = v.get(e);
      if (!c) {
      ...
      // @ts-expect-error - TS2322 - Type 'File | FSMap' is not assignable to type 'FSMap'.
      v = c;
    }
    return v;
  }
  // @ts-expect-error - TS2488 - Type 'any[] | undefined' must have a '[Symbol.iterator]()' method that returns an iterator.
