...
function shouldRunYarn(
  oldDeps: REPLOptions['dependencies'] | null | undefined,
  newDeps: REPLOptions['dependencies'],
) {
  if (oldDeps) {
...
export async function yarnInstall(
  ...
  if (await fs.exists('/app/package.json')) {
    ...
    if (deps) {
      dependencies = Object.entries(deps) as Array<[string, string]>;
    }
  }
