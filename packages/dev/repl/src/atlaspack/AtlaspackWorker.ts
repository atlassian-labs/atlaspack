...
export type BundleOutput = {
  ...
  sourcemaps: Map<string, string> | null | undefined;
} | BundleOutputError;
...
async function convertDiagnostics(inputFS: MemoryFS, diagnostics: Array<Diagnostic>) {
...
async function renderDiagnostics(inputFS: MemoryFS, diagnostics: Array<Diagnostic>): Promise<string> {
  return (
    await Promise.all(
...
async function bundle(assets: FSList, options: REPLOptions, progress: (arg1: string) => void): Promise<BundleOutput> {
...
async function watch(
  ...
  writeAssets: (arg1: FSList) => Promise<unknown>;
}> {
...
function sendMsg(target: MessagePort, type: any, data: any, transfer: undefined) {
  let id = uuidv4();
