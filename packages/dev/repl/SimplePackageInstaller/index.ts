...
async function findPackage(fs: FileSystem, dir: FilePath | string) {
...
export default class SimplePackageInstaller implements PackageInstaller {
  ...
  async _npmFetch(tarball: string): Promise<Map<string, Uint8Array>> {
    ...
    if (!res.arrayBuffer) {
      ...
      const buffer = await new Promise((resolve: (result: Promise<Buffer> | Buffer) => void) =>
        res.body.on('end', () => {
  ...
  async _getDependency(name: string, version: string): Promise<{
    ...
    return {
      name,
      // $FlowFixMe
      deps: (deps ? Object.entries(deps) : [] as Array<[string, string]>),
      files,
    };
  }
  ...
  async _installDependenciesInto(
    ...
    await Promise.all(
      dependencies.map(async ([name, version]: [any, any]) => {
  ...
  async init({
    ...
    if (pkgPath) {
      ...
      let dependencies: Array<[string, string]> = [
        ...
        .filter(Boolean)
        // $FlowFixMe
        .map((v) => (Object.entries(v) as Array<[string, string]>))
        .flat(1);
