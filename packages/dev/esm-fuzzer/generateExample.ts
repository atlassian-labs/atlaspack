...
function appendToModule(
  state: State /*: State*/,
  n: number /*: number*/,
  data: Partial<Module> /*: $Shape<Module> */,
) {
...
function getNewModuleIndex(state: State /*: State*/) /*: number*/ {
...
const ACTIONS /*: Array<[number, (State) => State]> */ = [
  [
    0.2,
    function addUnusedNamedExport(oldState: State) {
  ...
  [
    0.25,
    function addNamedExport(oldState: State) {
  ...
  [
    0.3,
    function moveLocalExportToNewDependency(state: State) {
  ...
  [
    0.2,
    function addNamespaceReexportToExisting(state: State) {
  ...
  [
    0.05,
    function makeSideEffectFree(state: State) {
...
function mutate(state: State /*: State */) /*: State*/ {
  let action;
...
function numberToFilename(n: number /*: number */, type: string /*: string*/) {
  return `${n}.${type}`;
}

function linearizeState(state: State /*: State */) /* : Fixture */ {
  let modules /*: Array<[string, Module]> */ = Object.entries(state.modules);
  return {
    files: Object.fromEntries(
      modules
        .map(([n, {type, imported, exported}]: [any, any]) => {
          let importStatements: Array<Statement> = [];
          let otherStatements: Array<Statement> = [];
          let exportStatements: Array<Statement> = [];
