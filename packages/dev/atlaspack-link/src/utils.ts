...
export async function findParcelPackages(fs: FileSystem, rootDir: string, files: Map<string, string> = new Map()): Promise<Map<string, string>> {
...
export function mapNamespacePackageAliases(ns: string, parcelPackages: Map<string, string>): Map<string, string> {
...
export async function cleanupNodeModules(root: string, predicate: (filepath: string) => boolean, opts: CmdOptions): Promise<void> {
  let {fs} = opts;
