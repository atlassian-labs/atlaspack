...
export class AtlaspackWatcherWatchmanJS implements Watcher {
  ...
  async getEventsSince(dir: string, snapshot: FilePath, opts?: Options): Promise<Event[]> {
    await this.init(dir);
  ...
  _createExpression(dir: string, ignore?: Array<FilePath | GlobPattern>): WatchmanArgs {
    ...
    if (ignore) {
      const customIgnores = ignore?.map((filePathOrGlob: FilePath | GlobPattern) => {
  ...
  async subscribe(dir: string, fn: SubscribeCallback, opts?: Options): Promise<AsyncSubscription> {
    await this.init(dir);
