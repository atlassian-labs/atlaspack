...
export default class NodeResolver {
  ...
  async resolve(options: ResolveOptions): Promise<ResolveResult | null | undefined> {
    // Special case
  ...
  async resolveBuiltin(name: string, options: ResolveOptions): Promise<ResolveResult | null | undefined> {
  ...
  async handleError(error: any, options: ResolveOptions): Promise<Diagnostic | Array<Diagnostic> | null | undefined> {
    switch (error.type) {
      case 'FileNotFound': {
  ...
  async checkExcludedDependency(sourceFile: FilePath, name: string, options: ResolveOptions): Promise<Diagnostic | null | undefined> {
    let [moduleName] = getModuleParts(name);
...
function environmentToExportsConditions(env: Environment, mode?: BuildMode | null): number {
