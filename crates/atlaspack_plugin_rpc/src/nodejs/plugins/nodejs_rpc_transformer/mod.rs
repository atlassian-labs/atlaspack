...
/// Record a bailout for a specific transformer.
fn record_transformer_bailout(transformer_name: &str) {
  let mut stats = TRANSFORMER_BAILOUT_STATS.write();
  *stats.entry(transformer_name.to_string()).or_insert(0) += 1;
}
...
impl TransformerPlugin for NodejsRpcTransformerPlugin {
  ...
  async fn transform(&self, asset: Asset) -> Result<TransformResult, Error> {
    ...
    if cache_bailout {
      // Record bailout for stats tracking (all transformers, including expected ones)
      record_transformer_bailout(&self.plugin_node.package_name);
