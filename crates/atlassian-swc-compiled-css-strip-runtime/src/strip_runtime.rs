...
use atlassian_swc_compiled_css::{SortOptions, sort_atomic_style_sheet};
...
impl StripRuntimeTransform {
  ...
  fn finalize_program(&mut self, program: &mut Program) {
    ...
    if let Some(extract) = self.config.options.extract_styles_to_directory.clone() {
      self.extract_styles_to_directory(program, extract);
    }
  }
  ...
  fn extract_styles_to_directory(
    ...
    let filename = self.config.filename.as_ref().unwrap_or_else(|| {
            panic!(
                "@compiled/babel-plugin-strip-runtime expected the filename not to be empty, but actually got '{}'",
    ...
    let source_file_name = self.config.source_file_name.as_ref().unwrap_or_else(|| {
      panic!("Source filename was not defined");
    });

    if !source_file_name.contains(&extract.source) {
      panic!(
    ...
    let relative =
      &source_file_name[source_file_name.find(&extract.source).unwrap() + extract.source.len()..];
    ...
    if let Some(parent) = css_path.parent() {
      create_dir_all(parent).expect("failed to create css directory");
    }
    ...
    write(&css_path, sorted).expect("failed to write extracted stylesheet");
