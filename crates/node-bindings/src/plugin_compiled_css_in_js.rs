...
const bar = 2; const str = "// not comment"; const tpl = `/* not comment */`; "#;
  ...
  #[test]
  fn collision_resistant_hash_matches_compiled_babel_output() {
    let mut config = create_test_config(true, false);
    config.config.collision_resistant_hash = Some(true);
    ...
    let code = indoc! {r#"
      ...
      const styles = css({
        ...
        backgroundColor: 'var(--ds-surface, rgba(255, 255, 255, 1))',
        gridColumn: '1 / -1',
        '&:hover': { opacity: 0.5 },
        '@media (min-width: 48rem)': { display: 'block' },
      });
      export const App = () => <div css={styles}>x</div>;
    "#};

    let transformed = process_compiled_css_in_js(code, &config)
      .expect("collision-resistant transformation should succeed");
    assert!(!transformed.bail_out, "transform should not bail out");

    // Generated by @compiled/babel-plugin with optimizeCss and
    // collisionResistantHash enabled. Pinning the full classes verifies the
    // Atlaspack Classic transform uses the same group and value hash seeds,
    // not merely the same 11-character shape.
    let expected_classes = [
      "_2j3gqAg8pm", // grid-column: 1 / -1
      "_1UtDYzGowl", // color: red
      "_0Of8r2NHnM", // padding-top token fallback
      "_0KLXrulfZc", // background-color rgba fallback
      "_2OgLIsXvJ3", // &:hover opacity
      "_2UeR8bvLZJ", // media-query display
    ];
    let output = format!(
      "{}\n{}",
      transformed.code,
      transformed.style_rules.join("\n")
    );
    for class_name in expected_classes {
      assert!(
        output.contains(class_name),
        "expected Babel-compatible class {class_name} in transformed output:\n{output}"
      );
    }
  }
