...
impl TransformerPlugin for AtlaspackCssTransformerPlugin {
  async fn transform(
    ...
    asset.code = Box::new(Code::from(css_code.join("\n")));
...
mod tests {
  ...
  async fn supports_css_imports() {
    let asset = Asset {
      ...
      code: Box::new(Code::from("@import './stuff.css';")),
      ..Default::default()
    };
  ...
  async fn supports_css_modules() {
    let asset = Asset {
      ...
      is_source: true,
      code: Box::new(Code::from(".root {display: 'block'}")),
      ..Default::default()
    };
    ...
    assert_eq!(
      ...
      AssetWithDependencies {
        asset: Asset {
          ...
          code: Box::new("module.exports[\"root\"] = `EcQGha_root`;\n".into()),
          file_path: "styles.module.css".into(),
          is_source: true,
          ..Default::default()
        },
