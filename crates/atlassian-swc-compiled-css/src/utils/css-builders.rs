    ...
    if matches!(
      ...
      let css = result
        ...
        .map(|item| match item {
          ...
          // COMPAT(AFB-1871): Sheet items (e.g. @keyframes rules) are always hoisted
          // to module scope and do not participate in runtime class-name toggling.
          // Wrapping them in a Logical item would cause transform_css to receive the
          // raw @keyframes CSS as a property declaration, producing empty sheets.
          // Babel preserves Sheet items through logical expressions unchanged.
          CssItem::Sheet(_) => item,
...
mod tests {
  ...
  #[test]
  fn logical_and_preserves_sheet_items_from_css_call_with_keyframes() {
    // When `condition && css({ '@keyframes foo': {...}, svg: {...} })` is processed,
    // the keyframes extraction produces CssItem::Sheet for the @keyframes rule and
    // CssItem::Unconditional for the animation-name property. The Sheet item must
    // be preserved as-is (not wrapped in Logical) because sheets are always hoisted
    ...
    let sheet_css = "@keyframes k123{0%{color:#253858}to{color:currentColor}}".to_string();
    let unconditional_css = "animation-name:k123".to_string();

    let right_output = CssOutput {
      css: vec![
        CssItem::Sheet(SheetCssItem {
          css: sheet_css.clone(),
        }),
        CssItem::Unconditional(UnconditionalCssItem {
          css: unconditional_css.clone(),
        }),
      ],
      variables: Vec::new(),
    };

    // Simulate what build_css_internal does for Expr::Bin(LogicalAnd):
    // It maps each item through the match arms
    let expression = parse_expression("props.isAnimating");
    let css: Vec<CssItem> = right_output
      .css
      .into_iter()
      .map(|item| match item {
        CssItem::Logical(mut logical) => {
    ...
    // Verify: Sheet item is preserved as CssItem::Sheet, not wrapped in Logical
    assert_eq!(css.len(), 2, "Should have 2 CSS items");
    assert!(
      matches!(&css[0], CssItem::Sheet(s) if s.css == sheet_css),
      "First item should be a Sheet with the keyframe CSS, got: {:?}",
      css[0]
    );
    assert!(
      matches!(&css[1], CssItem::Logical(l) if l.css == unconditional_css),
      "Second item should be a Logical wrapping the animation-name, got: {:?}",
      css[1]
    );
  }
}
