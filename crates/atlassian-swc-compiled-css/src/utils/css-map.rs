...
impl ErrorMessages {
  fn message(&self) -> &'static str {
    match self {
            ...
            ErrorMessages::SelectorBlockWrongPlace => {
...
pub fn error_if_not_valid_object_property(property: &PropOrSpread, _meta: &Metadata) {
  match property {
    PropOrSpread::Prop(prop) => match &**prop {
      Prop::Method(_) | Prop::Getter(_) | Prop::Setter(_) => {
        panic!(
          "{}",
          create_error_message(ErrorMessages::NoObjectMethod.message())
        );
      }
      _ => {}
    },
    PropOrSpread::Spread(_) => {
      panic!(
        "{}",
        create_error_message(ErrorMessages::NoSpreadElement.message())
      );
    }
  }
}
