...
use crate::utils_css_map::{
  ErrorMessages, create_error_message, error_if_not_valid_object_property,
};
  ...
  F: FnMut(&Expr, &Metadata) -> CssOutput,
{
  match usage {
    CssMapUsage::TaggedTemplate(_) => {
      panic!(
        "{}",
        create_error_message(ErrorMessages::NoTaggedTemplate.to_string())
      );
    }
    CssMapUsage::Call(call_expr) => {
      let Some(binding_identifier) = parent_identifier else {
        panic!(
          "{}",
          create_error_message(ErrorMessages::DefineMap.to_string())
        );
      };
      ...
      if call_expr.args.len() != 1 {
        panic!(
          "{}",
          create_error_message(ErrorMessages::NumberOfArgument.to_string())
        );
      }
      ...
      if argument.spread.is_some() {
        panic!(
          "{}",
          create_error_message(ErrorMessages::ArgumentType.to_string())
        );
      }
      ...
      for property in &object_lit.props {
        ...
        let Prop::KeyValue(key_value) = prop.as_ref() else {
          panic!(
            "{}",
            create_error_message(ErrorMessages::StaticVariantObject.to_string())
          );
        };
