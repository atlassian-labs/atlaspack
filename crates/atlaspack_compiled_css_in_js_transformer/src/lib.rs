...
use swc_core::ecma::transforms::compat::es2015::classes;
use swc_ecma_ast::*;
...
pub struct AtomicCssCollector {
  ...
  // Separate collection of generated CSS rules for external consumption
  pub generated_css_rules: Vec<String>,
  ...
  // Track CSS variable declarations that should be removed from the final output
  css_vars_to_remove: FxHashSet<Atom>,
}
...
impl VisitMut for AtomicCssCollector {
  fn visit_mut_module(&mut self, m: &mut Module) {
    ...
    if !self.wrapper_key_to_rules.is_empty() {
      ...
      for key in keys {
        if let Some(chunks) = self.wrapper_key_to_rules.remove(&key) {
          ...
          // Also add to the separate collection for external consumption
          self.generated_css_rules.push(inner.clone());
          // Hoist as a const
          self.next_id += 1;
    ...
    // Remove CSS variable declarations after processing
    if !self.css_vars_to_remove.is_empty() {
      // Filter out variable declarators that are marked for removal
      for item in &mut m.body {
        if let ModuleItem::Stmt(Stmt::Decl(Decl::Var(var_decl))) = item {
          var_decl.decls.retain(|decl| {
            if let Pat::Ident(BindingIdent { id, .. }) = &decl.name {
              !self.css_vars_to_remove.contains(&id.sym)
            } else {
              true
            }
          });
      ...
      // Remove empty variable declarations after filtering out CSS variables
      m.body.retain(|item| {
        if let ModuleItem::Stmt(Stmt::Decl(Decl::Var(var_decl))) = item {
                        ...
                        if let Some(arg0) = call.args.get(0) {
                          if let Expr::Object(outer) = &*arg0.expr {
                            ...
                            if !per_key.is_empty() {
                              ...
                              // Mark this variable for removal since we've extracted the CSS
                              self.css_vars_to_remove.insert(id.sym.clone());
                            }
                          }
                        }
                      }
