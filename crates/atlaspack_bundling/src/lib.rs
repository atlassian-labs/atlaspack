...
impl Bundler for MonolithicBundler {
  fn bundle(
    ...
    // Find all entry dependencies + entry assets.
    let mut entries: Vec<(atlaspack_core::types::Dependency, String)> = Vec::new();
      ...
      if let Some(dep_node_id) = asset_graph.get_node_id_by_content_key(&dep.id) {
        ...
        for n in neighbors {
          if let Some(AssetGraphNode::Asset(asset)) = asset_graph.get_node(&n) {
            entries.push(((**dep).clone(), asset.id.clone()));
            break;
          }
        }
      }
    ...
    for (entry_dep, entry_asset_id) in entries {
      ...
      // Build a monolithic JS bundle per entry.
      ...
      while let Some(asset_id) = stack.pop() {
        ...
        if let Some(node_id) = asset_graph.get_node_id_by_content_key(&asset_id) {
          ...
          {
            let asset_bundle_node_id = *bundle_graph
              .get_node_id_by_content_key(&asset.id)
              .ok_or_else(|| anyhow::anyhow!("Missing asset node id in bundle graph"))?;

            bundle_graph.add_edge(
              &bundle_node_id,
              &asset_bundle_node_id,
              NativeBundleGraphEdgeType::Contains,
            );
            bundle_graph.add_edge(
              &bundle_node_id,
            ...
            if let Some(child_node) = asset_graph.get_node(&child) {
              match child_node {
                AssetGraphNode::Dependency(dep) => {
                  let dep_node_id = *bundle_graph
                    .get_node_id_by_content_key(&dep.id)
                    .ok_or_else(|| anyhow::anyhow!("Missing dependency node id in bundle graph"))?;
                  bundle_graph.add_edge(
                    &bundle_node_id,
                    &dep_node_id,
                    NativeBundleGraphEdgeType::Contains,
